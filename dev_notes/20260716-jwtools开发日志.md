...
- **v1** (2025-XX-XX): 从 LungAgingERV 的 Ext Fig1F 脚本泛化而来
- **设计动机**：同一套"donor-level比例+两两Wilcoxon+dataset overlay"
  逻辑在 LungAgingERV / LungERVIPF / PH-ERVAtlas 中反复出现，
  遂抽象为参数化函数
- **已知限制**：sample_col假设每个sample只属于唯一dataset，
  多dataset归属场景需要先跑健全性检查（见函数文档Notes）
- **调用示例**：见各项目 z6_2 系列脚本
...
# 2: 整合为 jwtools 函数：`ct_proportion_analysis()`
